...
-- leads テーブル (ai_investigation カラムは削除)
...
CREATE TABLE IF NOT EXISTS public.ai_artifacts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  kind text NOT NULL, -- 'research', 'proposal', 'estimate', 'mail', 'analysis'
  title text NOT NULL,
  lead_id uuid REFERENCES public.leads(id) ON DELETE SET NULL,
  customer_id uuid REFERENCES public.customers(id) ON DELETE SET NULL,
  body_md text, -- 本文(MarkdownやJSON文字列)
...
-- 'ai_artifacts' テーブルのトリガー
DROP TRIGGER IF EXISTS on_ai_artifacts_update ON public.ai_artifacts;
CREATE TRIGGER on_ai_artifacts_update
BEFORE UPDATE ON public.ai_artifacts
FOR EACH ROW
EXECUTE FUNCTION public.touch_updated_at();

-- 'estimates' テーブルのトリガー
DROP TRIGGER IF EXISTS trg_touch_estimates ON public.estimates;
CREATE TRIGGER trg_touch_estimates
...
DROP POLICY IF EXISTS "ai_artifacts_read_all" ON public.ai_artifacts;
DROP POLICY IF EXISTS "ai_artifacts_write_all" ON public.ai_artifacts;
DROP POLICY IF EXISTS "ai_artifacts_write_all_update" ON public.ai_artifacts;
DROP POLICY IF EXISTS estimates_all_auth ON public.estimates;
...
-- AI成果物テーブルのポリシー
CREATE POLICY "ai_artifacts_read_all" ON public.ai_artifacts FOR SELECT TO authenticated USING (true);
CREATE POLICY "ai_artifacts_write_all" ON public.ai_artifacts FOR INSERT TO authenticated WITH CHECK (true);
CREATE POLICY "ai_artifacts_write_all_update" ON public.ai_artifacts FOR UPDATE TO authenticated USING (true) WITH CHECK (true);

-- estimates テーブルのポリシー
CREATE POLICY estimates_all_auth ON public.estimates FOR ALL TO authenticated USING (true) WITH CHECK (true);

-- Storageバケットの作成とポリシー
INSERT INTO storage.buckets (id, name, public) VALUES ('ai','ai', true), ('inbox', 'inbox', false), ('receipts', 'receipts', false)
ON CONFLICT (id) DO NOTHING;

-- 古いポリシーを削除
DROP POLICY IF EXISTS "ai_artifacts_read_public" ON storage.objects;
DROP POLICY IF EXISTS "ai_artifacts_write_auth" ON storage.objects;

-- 新しいポリシーを（存在すれば削除してから）作成
DROP POLICY IF EXISTS ai_read_public ON storage.objects;
DROP POLICY IF EXISTS ai_write_auth ON storage.objects;
DROP POLICY IF EXISTS ai_update_auth ON storage.objects;

CREATE POLICY ai_read_public ON storage.objects FOR SELECT USING (bucket_id = 'ai');
CREATE POLICY ai_write_auth ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'ai');
CREATE POLICY ai_update_auth ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = 'ai') WITH CHECK (bucket_id = 'ai');

DROP POLICY IF EXISTS inbox_auth_all ON storage.objects;
CREATE POLICY inbox_auth_all ON storage.objects FOR ALL TO authenticated USING (bucket_id = 'inbox') WITH CHECK (bucket_id = 'inbox');

DROP POLICY IF EXISTS receipts_auth_all ON storage.objects;
CREATE POLICY receipts_auth_all ON storage.objects FOR ALL TO authenticated USING (bucket_id = 'receipts') WITH CHECK (bucket_id = 'receipts');

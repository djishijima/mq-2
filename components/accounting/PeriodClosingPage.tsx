...
import { Toast, ClosingChecklistItem, Job, ApplicationWithDetails, Page, JournalEntry, EmployeeUser } from '../../types';
...
const PeriodClosingPage: React.FC<PeriodClosingPageProps> = ({ addToast, jobs, applications, journalEntries, onNavigate, currentUser }) => {
  ...
  const handleMonthlyClose = async () => {
    ...
    try {
        ...
        // FIX: Pass currentUser.id as the sixth argument
        const summary = await generateClosingSummary('月次', currentMonthJobs, prevMonthJobs, currentMonthJournal, prevMonthJournal, currentUser.id);

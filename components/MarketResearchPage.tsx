...
import { Toast, MarketResearchReport, EmployeeUser } from '../types';
...
const MarketResearchPage: React.FC<MarketResearchPageProps> = ({ addToast, isAIOff, currentUser }) => {
  ...
  const handleResearch = async () => {
    ...
    setReport(null);
    try {
        // FIX: Pass currentUser.id as the second argument to generateMarketResearchReport
        const result = await generateMarketResearchReport(topic, currentUser.id);
        setReport(result);
        if (currentUser) {
            try {
                await dataService.addAIArtifact({
                    kind: 'research',
                    title: result.title,
                    body_md: JSON.stringify(result, null, 2),
                    created_by: currentUser.id,
                });
                addToast('調査レポートをAIドキュメントに保存しました。', 'success');
            } catch (artifactError) {
                console.error("Failed to save research artifact:", artifactError);
                addToast('調査レポートをAIドキュメントに保存できませんでした。', 'error');
            }
        }
    } catch (e) {

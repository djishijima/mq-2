...
interface ApplicationDetailModalProps {
    ...
    onApprove: (app: ApplicationWithDetails, reason: string) => Promise<void>;
...
const ApplicationDetailModal: React.FC<ApplicationDetailModalProps> = ({
    ...
    const handleApprove = async () => {
        ...
        await onApprove(application, comment);
        if (mounted.current) {
    ...
    const handleReject = async () => {
        ...
        setIsProcessing(true);
        await onReject(application, comment);
        if (mounted.current) {
            setIsProcessing(false);
        }
    };
    ...
    const customersById = new Map(customers.map(c => [c.id, c.customerName]));
    const jobsById = new Map(jobs.map(j => [j.id, j.title]));
    ...
    const getTargetName = (target: string) => {
        ...
        if (type === 'customer') return `顧客: ${customersById.get(id) || '不明'}`;
        if (type === 'job') return `案件: ${jobsById.get(id) || '不明'}`;
        return 'N/A';
    }
                ...
                <div className="p-6 overflow-y-auto space-y-6">
                    ...
                    <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
                         ...
                         <dl className="grid grid-cols-2 gap-x-4 gap-y-4">
                            ...
                            {code === 'EXP' && <>
                                <DetailItem label="経費明細" className="md:col-span-2">
                                    <div className="space-y-2">
                                    {Array.isArray(formData.details) && formData.details.map((d: any, i: number) => 
                                        <div key={i} className="text-sm p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg flex gap-4 items-start">
                                            <div className="flex-1">
                                                <p><span className="font-semibold">{d.paymentDate}:</span> {d.description} (<span className="font-mono font-bold">{d.costType}</span>)</p>
                                                {d.allocationTarget && <p className="text-xs text-slate-500">関連先: {getTargetName(d.allocationTarget)}</p>}
                                                <p className="font-bold">¥{d.amount?.toLocaleString()}</p>
                                            </div>
                                            {d.receiptUrl && (
                                                <a href={d.receiptUrl} target="_blank" rel="noopener noreferrer">
                                                    <img src={d.receiptUrl} alt="領収書" className="w-24 h-24 object-cover rounded-md border border-slate-200 dark:border-slate-600" />
                                                </a>
                                            )}
                                        </div>
                                    )}
